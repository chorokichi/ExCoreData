...
extension ExCoreDataSet {
    ...
    fileprivate static func factoryInstance(requiredData lData: DKCoreData.RequiredData, didConfiguration:@escaping (ExCoreDataResult<ExCoreDataSet, Error>) -> Void) {
        
        let callDidConfigurationOnMain = { (result: ExCoreDataResult<ExCoreDataSet, Error>) -> Void in
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            do {
                ...
                callDidConfigurationOnMain(ExCoreDataResult.success(coreDataSet))
            } catch {
                callDidConfigurationOnMain(ExCoreDataResult.failure(error))
            }
        }
    }
